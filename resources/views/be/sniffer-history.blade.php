...
@php
  $title      = 'History Log';
  $breadcrumb = 'Sniffer / History Log';
@endphp
...
<div class="sniffer-topbar">
  <div class="sniffer-topbar-left">
    <div class="sniffer-topbar-title">
      <i class="fas fa-history" style="color:#9ca3af;font-size:13px;"></i>
  ...
  <div class="sniffer-topbar-right">
    <span class="meta-badge">
      Total: <strong>{{ number_format($flows->total()) }} records</strong>
    </span>

    <a href="{{ route('sniffer.history') }}" class="btn-hard-refresh">
...
<div class="stat-grid">
  <div class="stat-card">
    <div class="stat-card-top">
      <div>
        ...
        @endphp
        <div class="stat-number">{{ $tbHuman }}</div>
  ...
  <div class="stat-card">
    <div class="stat-card-top">
      <div>
        <div class="stat-number">{{ number_format($uniqueSrc) }}</div>
  ...
  <div class="stat-card">
    <div class="stat-card-top">
      <div>
        <div class="stat-number">{{ number_format($uniqueDst) }}</div>
...
<div class="card-shell" style="margin-bottom:14px;">
  ...
  <div class="filter-form-wrap">
    <form method="GET" action="{{ route('sniffer.history') }}" class="filter-row">
      ...
      {{-- Start Date --}}
      <input type="date" name="start_date" class="filter-select" value="{{ $start_date ?? '' }}">

      {{-- End Date --}}
      <input type="date" name="end_date" class="filter-select" value="{{ $end_date ?? '' }}">

      <button type="submit" class="btn-apply">Apply</button>
      <a href="{{ route('sniffer.history') }}" class="btn-clear">
...
<div class="card-shell">
  <div class="card-head">
    ...
    <div class="table-meta">
      <form method="GET" action="{{ route('sniffer.history') }}" class="perpage-row">
    ...
    <table id="flow-table" class="table align-middle">
      ...
      <tbody id="flow-tbody-history">
        ...
        <tr onclick="showHistoryDetail({{ json_encode($flow) }}, this)">
          ...
          <td class="td-info">{{ $flow->column_info ?: '—' }}</td>
        </tr>
        @empty
        <tr>
          <td colspan="8">
...
function showHistoryDetail(flow, rowEl) {
  const rows = document.querySelectorAll('#flow-tbody-history tr');
